...
|---|---|---|
| **Policy (OPA/Rego)** | OPA-evaluated + audited on **both** the REST path (`/api/v1/tools/{id}/invoke`) and the `/mcp` invoke path (both call `services/invocation.py`); built-in `/mcp` **meta-tools now evaluate OPA under the real caller identity** (role-based `platform_meta_tool_roles` in `authz.rego`, 6.1 — no longer a placeholder `platform_admin`) | The **discovery==invoke** invariant is enforced on the catalog but **not** yet on `/mcp` invoke (an admin role can invoke any tool by name — see `mcp_server.py`, 6.2 in progress); **signed OPA bundles are an opt-in overlay** (`docker-compose.opa-signed.yml`), not the default |
