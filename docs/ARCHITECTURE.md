...
| 🎫 **One OAuth issuer identity (RFC 9207)** | `authorization_servers`, the AS-metadata `issuer`, and the RFC 9207 callback `iss` are all the **same realm issuer URL**. The proxy fronts *filtered* discovery **at that issuer path**, not by advertising its own origin as a separate AS. | A split issuer (proxy origin vs realm URL) is an RFC 9207 violation. A **spec-correct** client (Codex ≥0.143) rightly rejects it ("missing required issuer"); a **lenient** client (Claude Code) only "works" by ignoring the mismatch — so the bug hides until a stricter agent connects. "It works in Claude Code" is not evidence of compliance. | [`10-codex-oauth-issuer-consistency.md`](spec/10-codex-oauth-issuer-consistency.md) |
