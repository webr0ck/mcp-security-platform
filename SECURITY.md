...
|---|---|---|---|
| F-001 | Identity (mTLS) | The proxy trusts the gateway-set `X-Client-Cert-CN` header. This is only safe when the Nginx gateway is the **sole** network path to the proxy. A workload that can reach the proxy directly on the internal network could forge the header. | Set `GATEWAY_SECRET`; enforce network isolation so only the gateway reaches the proxy (`make security-check` includes an F-001 isolation gate — note it scans `docker-compose.yml`, validate your deployed tier separately). Defense-in-depth fix tracked on the roadmap. |
