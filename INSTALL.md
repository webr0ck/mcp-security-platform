...
This platform runs in one of two fundamentally different ways. They are **not**
interchangeable — pick the one that matches your goal.

| | **Production-shaped tiers** (this guide) | **Lab** ([LAB.md](LAB.md)) |
|---|---|---|
| Runtime | Docker Compose | Podman |
| IdP | **Bring your own** (engine), or bundled Keycloak in dev mode (standard/poc) | Self-contained Keycloak + Dex + Mock IdP, auto-seeded |
| Secrets | You fill `.env`; the init script generates the rest | All auto-generated by `scripts/lab-init.sh` |
| MCP servers | You onboard your own | 9 demo servers pre-wired |
| Use for | Running as a real security proxy in front of your infra | Evaluation, development, testing |

> **⚠️ `make up` is NOT a production target.** Bare `make up` / `make setup` start the
> **lab base stack** (`docker-compose.yml` only) — a partial stack with no IdP — and will
> sit unhealthy if used standalone. For production use the tier compose files below; for the
> lab use `make lab-up`.

### Production-shaped tiers (configure your own IdP + secrets)

Three tiers build on each other via Compose `include:`:

| Tier | Compose file | Init script | Adds | IdP |
|---|---|---|---|---|
| **engine** | `compose.engine.yml` | `scripts/init-engine.sh` | gateway, proxy, OPA, Postgres, Redis, Vault, step-ca | **external — bring your own** |
| **standard** | `compose.standard.yml` | `scripts/init-standard.sh` | + Keycloak, Grafana, Loki | bundled Keycloak (dev mode — **not for prod**) |
| **poc** | `compose.poc.yml` | `scripts/init-poc.sh` | + Wazuh SIEM, demo MCP servers + users | bundled Keycloak (dev mode — **not for prod**) |

```bash
# Engine — bring your own IdP (full walkthrough in "Quick install — engine tier" below)
cp deployments/engine/.env.example .env && bash scripts/init-engine.sh
docker compose -f compose.engine.yml up -d

# Standard — bundled Keycloak (dev mode)
cp deployments/standard/.env.example .env && bash scripts/init-standard.sh
docker compose -f compose.standard.yml up -d

# Full POC — Wazuh + demo MCP servers + alice/bob/carol
cp deployments/poc/.env.example .env && bash scripts/init-poc.sh
docker compose -f compose.poc.yml up -d
```

Each init script is safe to re-run (it never overwrites existing secrets) and generates only
the secrets it owns; you must still fill the operator-supplied values (`DB_PASSWORD`,
`PROXY_SECRET_KEY`, `VAULT_TOKEN`, etc.) in `.env` first.

> Standard and POC bundle Keycloak in `start-dev` mode (in-memory) — convenient for evaluation,
> but **realm + users are lost on restart**. For anything real, run the engine tier against an
> external IdP — see [Your own IDP (OIDC)](#your-own-idp-oidc).

### Lab (self-contained, auto-seeded — Podman)
...
0. [Choosing a run mode](#choosing-a-run-mode)
